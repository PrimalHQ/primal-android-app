...
class FeedMapperTest {
    ...
    fun isLatestFeed_returnsTrueForLatestFeed() {
        Feed(name = "Latest", directive = userId).isLatest(userId) shouldBe true
    }

    @Test
    fun isLatestFeed_ignoresFeedNameAndReturnsTrue() {
        Feed(name = "XYZ", directive = userId).isLatest(userId) shouldBe true
    }

    @Test
    fun isLatestFeed_returnsFalseForLatestWithRepliesFeed() {
        Feed(name = "Latest", directive = "$LWR_DIRECTIVE_PREFIX;$userId").isLatest(userId) shouldBe false
    }

    @Test
    fun isLatestFeed_returnsFalseForOtherFeed() {
        Feed(name = "Latest", directive = "global;trending").isLatest(userId) shouldBe false
    }

    @Test
    fun isLatestWithReplies_returnsTrueForLatestWithRepliesFeed() {
        Feed(name = "LWR", directive = "$LWR_DIRECTIVE_PREFIX;$userId").isLatestWithReplies(userId) shouldBe true
    }

    @Test
    fun isLatestWithReplies_ignoresFeedNameAndReturnsTrue() {
        Feed(name = "XYZ", directive = "$LWR_DIRECTIVE_PREFIX;$userId").isLatestWithReplies(userId) shouldBe true
    }

    @Test
    fun isLatestWithReplies_returnsFalseForLatestFeed() {
        Feed(name = "LWR", directive = userId).isLatestWithReplies(userId) shouldBe false
    }

    @Test
    fun isLatestWithReplies_returnsFalseForOtherFeed() {
        Feed(name = "LWR", directive = "global;trending").isLatestWithReplies(userId) shouldBe false
    }
    ...
    fun asContentFeedData_latestFeedIsProperlyMapped() {
        val latestFeed = Feed(name = "Latest", directive = userId)
    ...
    fun asContentFeedData_latestWithRepliesFeedIsProperlyMapped() {
        val lwrFeed = Feed(name = "LWR", directive = userId.toLatestWithRepliesDirective())
    ...
    fun asContentFeedData_trendingFeedIsProperlyMapped() {
        val trendingFeed = Feed(name = "Trending 24h", directive = "global;trending")
