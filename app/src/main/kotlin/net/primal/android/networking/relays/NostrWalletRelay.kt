...
@Singleton
class NostrWalletRelayPool @Inject constructor(
    ...
    private var walletConnectPool = listOf<NostrSocketClient>()
    ...
    suspend fun publishEvent(nostrEvent: NostrEvent) {
        ...
        walletConnectPool.forEach { nostrSocketClient ->
            scope.launch {
    ...
    suspend fun append(relay: Relay) {
        ...
        createWalletConnectClientsPool(relays)
    }
        ...
        activeAccountStore.activeAccountState.collect {
            when (it) {
                is ActiveUserAccountState.ActiveUserAccount -> {
                    ...
                    createWalletConnectClientsPool(relays = relays)
                }

                ActiveUserAccountState.NoUserAccount -> {
                    clearWalletConnectClientsPool()
                }
            }
        }
    }
    ...
    private suspend fun createWalletConnectClientsPool(relays: List<Relay>) {
        clearWalletConnectClientsPool()
        poolMutex.withLock {
            walletConnectPool = relays
    ...
    private suspend fun clearWalletConnectClientsPool() = poolMutex.withLock {
        walletConnectPool.forEach { it.close() }
        walletConnectPool = emptyList()
    }
